

```

WM keybinds are all in my sxhkd config. they just use the base keys with MOD
2 places for arrow keys because im used to both for different things. Should problably change that soon.


Base
| Tab |  Q  |  W  |  E  |  R  |  T  |   |  Y  |  U   |  I  |  O  |  P  |  '   |
| ESC |  A  |  S  |  D  |  F  |  G  |   |  H  |  J   |  K  |  L  |BSPC |  /   |
|CTRL |  Z  |  X  |  C  |  V  |  B  |   |  N  |  M   |  @  |  ,  |  .  | Alt  |
                  | LWR |Shift|Space|   |Enter| RSE  | Mod |
                   
Symbols
| Tab |  1  |  2  |  3  |  4  |  5  |   |  6  |  7  |  8  |  0  |  9  |  `  |
| Alt |  !  |  &  |  =  |  *  |  +  |   |  {  |  (  |  [  |  ;  |  :  |  "  |
|Shift|  $  |  ?  |  _  |  /  |  -  |   |  }  |  )  |  ]  |  <  |  >  |  |  |
                  |CTRL | LWR |Space|   |Enter|-----|     |

Funcion
| Alt | F1  | F2  | F3  | F4  | F5  |   |  %  |     |  ^  |     |TILDE|  #  |
|CTRL | F6  | F7  | F8  | F9  | F10 |   |  ←  |  ↓  |  ↑  |  →  |     |  \  |
|Shift|     |     |     | F12 | F11 |   |     |     |     |     |     |     |
                  |LCTRL|-----| SPC |   |BTCLR|-----|     |

WM
| Tab |main |close |  obs  | misc | term |   |youtube|scrnshot| OCR |obsi |system |volUP|
| ESC | AI  |      |discord|krita | git  |   |   ←   |    ↓   |  ↑  |  →  | pause |volDN| 
|Shift|sxhkd|reboot| open  |      |browse|   |school | music  |kitty|     | back  |skip |
                   |       |      | code |   |       |        |-------|

Macro                (Transcription)                (Mouse)
| Alt |     |     |googl|inline| main |   |  ←   |  ↓  |  ↑  |  →  |     |      | 
|CTRL |     |     |     |      |to-do |   |  ←   |  ↓  |  ↑  |  →  |     |      | 
|Shift|     |     |     |      | task |   |      |     |     |     |     |      |
                  |-----|      |      |   | LCLK |RCLK |MCLK |
```
